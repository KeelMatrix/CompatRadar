...
- SDK: 8.0.425.
- Runtime/restore elapsed: restore 0.76 seconds; test corpus 112.51 seconds.
...
- Real repository probes: `KeelMatrix.Redaction` COMPATIBLE (exit 0, 23.60s); `KeelMatrix.Telemetry` COMPATIBLE (exit 0, 24.37s); `KeelMatrix.QueryWatch` COMPATIBLE (exit 0, 43.28s). Each candidate was evaluated in an isolated copy.
