...
JSON, and an unlabeled witness pack under `artifacts/`. It returns `0` only when every named check
...
## Repository probes and the reviewer pack
...
Those samples deliberately omit classifications, expectations, and every other outcome label so a
reviewer can judge attribution and reproduction usefulness without knowing which outcome the sample
had. `scripts/validate-witness-pack.ps1` checks only structural completeness and the absence of
labels; it never judges attribution, never reads expectation data, and never claims a review
outcome. The independent review is owned by the reviewer, not by the repository scripts.
